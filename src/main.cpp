...
uint256 hashGenesisBlock("0x00000809221920fdf584cc538344ab81411b0c388ac68f3fbd62db6c23c2bb2e"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // WinnerCoin: starting difficulty is 1 / 2^12
...
{
    // WinnerCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // WinnerCoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // WinnerCoin: 5 minutues
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 288
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // WinnerCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, WinnerCoin - DarkGravity v2, written by Evan Duffield - evan@darkcoins.net - Changed by 7vpo*/
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // WinnerCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // WinnerCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        hashGenesisBlock = uint256("0x000008248b5ee23cd4c57d299385c696b4f88cd7bc12461d7b033ac40a4a3bb2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block        
        const char* pszTimestamp = "CNN Middle East 16/Nov/2015 French jets bomb Syria in the ISIS stronghold of Raqqa";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 7 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041254710fa843ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1447637546;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1461922;

        if (fTestNet)
        {
            block.nTime    = 1447629047;
            block.nNonce   = 1521706;
        }
        ...
        block.print();
        //If you want to copy winnercoin, uncomment this part back in to create a new genesisblock
        //if (true && block.GetHash() != hashGenesisBlock)
                ...
                assert(block.hashMerkleRoot == uint256("0x9021a0a59fcaf7978c0543fcb48db7f0a94574549c9bf064ce630be7f0ed2c25"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WinnerCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WinnerCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WinnerCoinMiner(CWallet *pwallet)
{
    printf("WinnerCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("WinnerCoin-miner");
    ...
    try { loop {
        ...
        printf("Running WinnerCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WinnerCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WinnerCoinMiner, pwallet));
}
